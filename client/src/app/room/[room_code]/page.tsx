...
import { Crown, Copy, Link as LinkIcon, Upload, WifiOff, Search, Music2, Loader2, PlusCircle, Sparkles } from 'lucide-react';
...
const DisconnectedOverlay = () => (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="fixed bottom-32 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-red-500/10 border border-red-500/20 text-red-200 px-6 py-3 rounded-full backdrop-blur-md shadow-xl">
        <WifiOff size={18} className="animate-pulse" /><span className="font-medium text-sm">Reconnecting to server...</span>
    </motion.div>
);
...
const RoomSidebar = memo(function RoomSidebar({ roomCode, onOpenUpload, onOpenSearch }: { roomCode: string, onOpenUpload: () => void, onOpenSearch: () => void }) {
    ...
    // Fallback: If socket fails, assume I am admin if I am the only user or if I just created it
    // Real logic happens on server, but this unlocks the UI
    const effectiveAdmin = isAdmin || users.length <= 1;
    ...
    return (
        <aside className="hidden lg:flex flex-col gap-8 sticky top-8 h-[calc(100vh-8rem)]">
            <motion.div initial={{opacity: 0, x: 20}} animate={{opacity: 1, x: 0}}>
                <h2 className="text-xs font-bold tracking-widest text-gray-500 mb-3 uppercase">Session Link</h2>
                <div className="flex items-center gap-2 p-1 rounded-xl bg-white/5 border border-white/10 pr-2 group hover:border-white/20 transition-colors">
                    ...
                    <button onClick={copyToClipboard} className="p-2 rounded-lg hover:bg-white/10 transition-colors">
                        <Copy size={16} className={copied ? "text-cyan-400" : "text-gray-500"} />
                    </button>
                </div>
            </motion.div>
            ...
            <motion.div initial={{opacity: 0, x: 20}} animate={{opacity: 1, x: 0}} className="flex-1 flex flex-col min-h-0">
                <h2 className="text-xs font-bold tracking-widest text-gray-500 mb-3 uppercase flex justify-between">
                    <span>Listeners</span>
                    <span className="bg-white/10 px-2 rounded-full text-white">{users.length}</span>
                </h2>
                <div className="space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                    {users.map((user, idx) => (
                        <div key={idx} className="flex items-center gap-3 p-2 rounded-lg bg-white/5 border border-transparent hover:border-white/10 transition-colors">
                            <div className={`w-2 h-2 rounded-full ${user.isAdmin ? 'bg-amber-400 shadow-[0_0_8px_rgba(251,191,36,0.5)]' : 'bg-gray-600'}`} />
                            <span className="text-sm truncate text-gray-200 font-medium">{user.name}</span>
                            {user.isAdmin && <Crown size={12} className="ml-auto text-amber-400 flex-shrink-0" />}
                        </div>
                    ))}
                </div>
            </motion.div>
            
            {/* Always show controls if empty or admin */}
    ...
    <div className="min-h-screen bg-black text-white overflow-hidden font-sans selection:bg-cyan-500/30">
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 p-4 bg-black/80 backdrop-blur-xl border-b border-white/10 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowSearchModal(true)} className="p-2 rounded-full bg-cyan-600 text-white shadow-lg shadow-cyan-900/50 active:scale-95 transition-transform"><PlusCircle size={20}/></button>
          </div>
      </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 lg:p-10 space-y-2 custom-scrollbar">
                    ...
                    {state.playlist.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-20 text-center opacity-80">
                            ...
                            <button onClick={() => setShowSearchModal(true)} className="px-8 py-4 rounded-full bg-cyan-600 text-white font-bold hover:scale-105 transition-transform shadow-lg shadow-cyan-600/20">
                                Add First Song
                            </button>
                        </div>
                    ) : (
                        <div className="space-y-2">
                             {state.playlist.map((song, index) => (
                                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} key={index} onClick={() => playerControls.selectTrack(index)} 
                                    className={`group flex items-center gap-5 p-4 rounded-2xl transition-all border cursor-pointer ${state.currentTrackIndex === index ? 'bg-white/10 border-cyan-500/30 shadow-lg shadow-black/20' : 'bg-transparent border-transparent hover:bg-white/5'}`}>
                                    ...
                                    <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                                        <div className="p-2 rounded-full bg-white/10 hover:bg-white/20 text-white"><Music2 size={16} /></div>
                                    </div>
                                </motion.div>
                            ))}
                        </div>
