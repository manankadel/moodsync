...
// ==========================================
// ZERO-LATENCY CONSTANTS
// ==========================================
const NETWORK_JITTER_THRESHOLD = 0.05; // 50ms - ignore tiny drifts
const HARD_SYNC_THRESHOLD = 0.3; // 300ms - force sync if beyond this
const SOFT_SYNC_PAUSE_DURATION = 100; // ms to pause before resuming
const SYNC_CHECK_INTERVAL = 50; // Check sync every 50ms for uploads
const SERVER_CLOCK_SYNC_INTERVAL = 5000; // Sync server clock every 5s
...
export const useRoomStore = create<RoomState>((set, get) => ({
  ...
  connect: (roomCode, username) => {
    ...
    socket.on('connect', () => {
      console.log('✅ Connected with Zero-Latency Sync');
  ...
  initializePlayer: (domId) => {
    ...
    audioEl.addEventListener('timeupdate', () => {
      if (!get().isLoading && !get().isSeeking) {
  ...
  syncPlayerState: (state) => {
    const { player, audioElement, playlist, currentTrackIndex, isPlaying, volume, isCollaborative, equalizer, audioNodes, isSeeking, serverClockOffset } = get();
    ...
    if (state.currentTime !== undefined && state.serverTimestamp !== undefined) {
      ...
      const networkLatency = (clientTime - serverTime) / 2;
      const adjustedServerTime = state.currentTime + networkLatency;

      const playerTime = currentTrack?.isUpload ? targetPlayer.currentTime : targetPlayer.getCurrentTime?.() || 0;
      const drift = adjustedServerTime - playerTime;
      ...
      if (drift > HARD_SYNC_THRESHOLD) {
        console.log(`🎯 HARD SYNC: Client behind by ${drift.toFixed(3)}s. Seeking to ${adjustedServerTime.toFixed(2)}s`);
        if (currentTrack?.isUpload) {
          targetPlayer.currentTime = adjustedServerTime;
        } else {
          targetPlayer.seekTo(adjustedServerTime, true);
        }
      } else if (drift < -NETWORK_JITTER_THRESHOLD) {
        console.log(`🎯 SOFT SYNC: Client ahead by ${Math.abs(drift).toFixed(3)}s. Pausing briefly.`);
        targetPlayer.pause();
        setTimeout(() => {
          if (get().isPlaying) targetPlayer.play();
        }, SOFT_SYNC_PAUSE_DURATION);
      }
    }
