...
export default function Player({ 
    title, artist, isPlaying, volume, duration, 
    ...
    return (
        <motion.footer 
            initial={{ y: 100 }} 
            animate={{ y: 0 }} 
            className="fixed bottom-0 left-0 right-0 z-50 h-auto sm:h-28 bg-black/80 backdrop-blur-xl border-t border-white/10 p-2 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]"
        >
                ...
                <AnimatePresence>
                    {statusMessage && (
                        <motion.div 
                            ...
                            exit={{opacity: 0}} 
                            className="absolute -top-12 left-1/2 -translate-x-1/2 bg-cyan-600/90 text-white px-4 py-2 rounded-full text-xs font-bold shadow-lg flex items-center gap-2 z-50 whitespace-nowrap"
                        >
                ...
                <AnimatePresence>
                    {driftWarning && (
                        <motion.div 
                            ...
                            animate={{opacity: 1, y: 0}} 
                            exit={{opacity: 0}} 
                            className="absolute -top-12 right-1/2 translate-x-1/2 bg-amber-600/90 text-white px-3 py-1 rounded-full text-[10px] font-bold shadow-lg flex items-center gap-1 z-40 whitespace-nowrap"
                        >
                            <div className="w-1.5 h-1.5 bg-white rounded-full animate-pulse" /> {driftWarning}
                        </motion.div>
                    )}
                </AnimatePresence>
                    ...
                    <div className="flex flex-col items-center gap-2">
                        ...
                        <div className="w-full flex items-center gap-3 text-xs text-gray-400 max-w-xs sm:max-w-sm lg:max-w-md font-mono">
                            ...
                            <input 
                                ...
                                className="w-full h-1 bg-gray-700 rounded-full appearance-none cursor-pointer accent-cyan-400 hover:accent-cyan-300"
                            />
                    ...
                    <div className="relative flex items-center justify-end gap-2">
                        ...
                        <button 
                            ...
                            className={`p-2 rounded-full transition-colors ${
                                showEqualizer ? 'text-cyan-400 bg-white/10' : 'text-gray-400 hover:text-white'
                            }`}
                        >
                        ...
                        <div className="items-center gap-2 hidden sm:flex group">
                            ...
                            <input 
                                ...
                                className="w-20 h-1 bg-gray-700 rounded-full appearance-none accent-white cursor-pointer" 
                            />
                        </div>
                    </div>
                </div>
