...
const YT_URL_RE = /(?:youtube\.com\/watch\?v=|youtu\.be\/)([a-zA-Z0-9_-]{11})/;
...
export default function SearchModal({ isOpen, onClose, roomCode }: { isOpen: boolean; onClose: () => void; roomCode: string }) {
  ...
  const [urlTrack, setUrlTrack] = useState<SearchResult | null>(null);
  const { isAdmin, isCollaborative, userId } = useRoomStore();

  const isYtUrl = !!extractYtId(query);

  const handleSearch = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!query.trim()) return;

    // If it's a YouTube URL, fetch track info directly
    const ytId = extractYtId(query);
    if (ytId) {
      setError(null);
      setIsSearching(true);
      try {
        const res = await fetch(`${API_URL}/api/yt-info`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ url: `https://www.youtube.com/watch?v=${ytId}` }),
        });
        if (!res.ok) { setError('Could not fetch video info.'); return; }
        const info = await res.json();
        if (info.error) { setError(info.error); return; }
        setUrlTrack({ id: info.id, title: info.title, artist: info.artist, thumbnail: info.thumbnail });
        setResults([]);
            ...
            <div className="p-4 border-b border-white/10 flex gap-3 items-center bg-black/40">
              ...
              <form onSubmit={handleSearch} className="flex-1">
                <input
                  autoFocus
                  value={query}
                  onChange={e => { setQuery(e.target.value); setUrlTrack(null); }}
            ...
            <div className="max-h-[60vh] overflow-y-auto p-2">
              {isSearching ? (
                <div className="flex flex-col items-center justify-center p-8 gap-3">
                  ...
                  <p className="text-gray-400 text-sm">{isYtUrl ? 'Fetching video info...' : 'Searching...'}</p>
                </div>
              ) : urlTrack ? (
                <div className="p-2">
                  <p className="text-xs text-gray-500 mb-2 px-1">Add from URL</p>
                  <div className="flex items-center gap-3 p-3 bg-cyan-500/10 border border-cyan-500/30 rounded-lg">
                    {urlTrack.thumbnail && <img src={urlTrack.thumbnail} className="h-14 w-14 rounded object-cover bg-gray-800 flex-shrink-0" alt={urlTrack.title} />}
                    <div className="flex-1 min-w-0">
                      <p className="font-medium truncate text-white text-sm">{urlTrack.title}</p>
                      <p className="text-xs text-gray-400 truncate">{urlTrack.artist}</p>
                    </div>
                    <button onClick={() => addTrack(urlTrack)} disabled={!!addingId} className="p-2 bg-cyan-500 text-white rounded-full hover:bg-cyan-400 transition-all disabled:opacity-50 flex-shrink-0">
                      {addingId === urlTrack.id ? <Loader2 size={20} className="animate-spin" /> : <Plus size={20} />}
                    </button>
                  </div>
                </div>
              ) : results.length === 0 ? (
